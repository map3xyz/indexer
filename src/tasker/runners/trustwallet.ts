...
export async function getTokenlistFromTrustWallet(network: string): Promise<TokenList> {
    ...
    try {
        ...
        const tokenlistFile = path.join(getTwaInstanceDirectory(), 'blockchains', network, 'tokenlist-extended.json.json');

        if(fs.existsSync(tokenlistFile)) {
            return JSON.parse(fs.readFileSync(tokenlistFile, 'utf8'));
        }
